...
# Your code here
files_list <- list.files(path="Timeseries_data/", pattern = "timeseries_data_", full.names = TRUE, recursive = T)
files_list
...
imputed_df <- combined_df %>%
  group_by(Date) %>% 
  mutate(Value = case_when(is.na(Value) ~ median(combined_df$Value, na.rm  = T),
                           TRUE ~ Value)) %>% 
...
filtered_june <- imputed_df %>%
  filter(Month == 'juni') %>% 
  group_by(Category) %>% 
  slice_(-c(which.min(imputed_df$Value), which.max(imputed_df$Value)))
...
#                  setNames(paste0(colnames(processed_df %>% separate(A:C)), "_", abbr))
#}))
